...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    setPetsOrder(prev => {
      ...
      if (filteredPrev.length === 1 && filteredPrev[0] === addCardId && missingIds.length > 0) {
        return [...missingIds, addCardId];
      }

      // Merge new IDs before the add-pet-card
      const addCardIndex = filteredPrev.indexOf(addCardId);
      if (addCardIndex !== -1) {
        const primary = filteredPrev.slice(0, addCardIndex);
  ...
  // Sync selectedPetId with top card
  useEffect(() => {
    if (petsOrder.length > 0) {
      const topId = petsOrder[0];
      if (topId !== 'add-pet-card') {
        if (selectedPetId !== topId) {
          setSelectedPetId(topId);
        }
      } else {
  ...
  const groupedActivities = useMemo(() => {
    ...
    return groups;
  }, [activities, historySelectedDate, historyViewMode, selectedPetId]);
      ...
      return (
        <div className="space-y-12">
            ...
            <div className="bg-zinc-50 p-5 rounded-2xl space-y-3">
              <div className="flex justify-between items-center border-b border-zinc-100/50 pb-3">
                <div className="flex gap-2 items-center">
                  ...
                  <span className="text-[10px] text-[#1A1A1A] font-semibold uppercase tracking-tight">오늘 총 {todayMeds.length}회 약 복용</span>
                </div>
                <span className="text-[10px] text-[#888888] font-medium">{new Date().toLocaleDateString('ko-KR', { month: 'long', day: 'numeric', weekday: 'short' })}</span>
              </div>
              {lastMed && (
                <div className="flex justify-between items-center">
                  <span className="text-[10px] text-[#888888]">마지막 복용</span>
                  <span className="text-[10px] text-[#1A1A1A] font-semibold uppercase font-mono">
          ...
          {recordStep === 1 ? (
              ...
              <div className="grid grid-cols-1 gap-3">
                {petMedications.length > 0 ? (
                  petMedications.map((med: any) => (
                    ...
                    >
                      <div className="space-y-1">
                        ...
                        <p className={`text-[10px] font-medium ${recordData.type === med.name ? 'text-[#888888]' : 'text-[#888888]'}`}>
                          {med.frequency} 복용 중
                        </p>
                      </div>
                  ...
                  <div className="bg-zinc-50/50 p-8 rounded-2xl border border-dashed border-zinc-100 flex flex-col items-center justify-center space-y-3">
                    ...
                    <p className="text-[10px] text-[#888888] uppercase tracking-widest">등록된 약이 없습니다</p>
                    <Link href={`/dashboard/edit-pet/${selectedPetId}`} className="text-[10px] text-[#1A1A1A] underline underline-offset-4 font-semibold">프로필에서 약 등록하기</Link>
                  </div>
                )}
                <button
                  onClick={() => {
            ...
            <div className="space-y-10">
              <div className="space-y-4">
                <span className="text-[10px] text-[#888888] uppercase tracking-widest">메모</span>
          ...
          {(lastFeed || todayMeals.length > 0) && (
            <div className="bg-zinc-50 p-5 rounded-2xl space-y-3">
              <div className="flex justify-between items-center border-b border-zinc-100/50 pb-3">
                <div className="flex gap-2 items-center">
                  <div className="w-1 h-1 rounded-full bg-black" />
                  <span className="text-[10px] text-[#1A1A1A] font-semibold uppercase tracking-tight">오늘 총 {todayMeals.length}회 급여</span>
                </div>
                <span className="text-[10px] text-[#888888] font-medium">{new Date().toLocaleDateString('ko-KR', { month: 'long', day: 'numeric', weekday: 'short' })}</span>
              </div>
              {lastFeed && (
                <div className="flex justify-between items-center">
                  <span className="text-[10px] text-[#888888]">마지막 급여</span>
                  <span className="text-[10px] text-[#1A1A1A] font-semibold uppercase font-mono">
      ...
      return (
        <div className="space-y-12">
          {(lastAction || todayActions.length > 0) && (
            <div className="bg-zinc-50 p-5 rounded-2xl space-y-3">
              <div className="flex justify-between items-center border-b border-zinc-100/50 pb-3">
                <div className="flex gap-2 items-center">
                  <div className="w-1 h-1 rounded-full bg-black" />
                  <span className="text-[10px] text-[#1A1A1A] font-semibold uppercase tracking-tight">오늘 총 {todayActions.length}회 {activityLabel}</span>
                </div>
                <span className="text-[10px] text-[#888888] font-medium">{new Date().toLocaleDateString('ko-KR', { month: 'long', day: 'numeric', weekday: 'short' })}</span>
              </div>
              {lastAction && (
                <div className="flex justify-between items-center">
                  <span className="text-[10px] text-[#888888]">마지막 {activityLabel}</span>
                  <span className="text-[10px] text-[#1A1A1A] font-semibold uppercase font-mono">
          ...
          {recordStep === 1 ? (
            <div className="space-y-10">
              <div className="space-y-4">
                <span className="text-[10px] text-[#888888] uppercase tracking-widest">소요 시간</span>
            ...
            <div className="space-y-10">
              <div className="space-y-4">
                <span className="text-[10px] text-[#888888] uppercase tracking-widest">반응</span>
            ...
            <div className="space-y-10">
              ...
              <div className="flex gap-3 pt-6">
                ...
                <button disabled={isSubmitting} onClick={handleRecordActivity} className="flex-[2] py-4 text-sm font-medium bg-black text-white rounded-xl active:scale-[0.98] transition-all disabled:bg-zinc-100 disabled:text-[#888888]">
                  {isSubmitting ? "기록 중..." : "기록 완료"}
                </button>
              </div>
            </div>
        ...
        <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
          {(lastTreat || todayTreats.length > 0) && (
            <div className="bg-zinc-50 p-5 rounded-2xl space-y-3">
              <div className="flex justify-between items-center border-b border-zinc-100/50 pb-3">
                <div className="flex gap-2 items-center">
                  <div className="w-1 h-1 rounded-full bg-black" />
                  <span className="text-[10px] text-[#1A1A1A] font-semibold uppercase tracking-tight">오늘 총 {todayTreats.length}회 간식</span>
                </div>
                <span className="text-[10px] text-[#888888] font-medium">{new Date().toLocaleDateString('ko-KR', { month: 'long', day: 'numeric', weekday: 'short' })}</span>
              </div>
              {lastTreat && (
                <div className="flex justify-between items-center">
                  <span className="text-[10px] text-[#888888]">마지막 간식</span>
                  <span className="text-[10px] text-[#1A1A1A] font-semibold uppercase font-mono">
          ...
          {recordStep === 1 ? (
            ...
            <div className="space-y-10 text-center">
              <div className="space-y-4">
                <h2 className="text-2xl font-light tracking-tight">얼마나 주셨나요?</h2>
                <p className="text-[11px] text-[#1A1A1A] font-semibold pt-1">"{recordData.memo}"</p>
              </div>
              <div className="space-y-8">
                <div className="flex flex-col items-center space-y-6">
                  ...
                  <div className="flex gap-2">
                    {['개', 'g', '컵'].map((unit) => (
                      <button key={unit} onClick={() => setRecordData({ ...recordData, amountUnit: unit })} className={`px-5 py-2 text-[10px] font-semibold tracking-widest border transition-all rounded-full ${(recordData.amountUnit || '개') === unit ? 'border-black bg-black text-white' : 'border-zinc-100 text-[#888888] hover:border-zinc-200'}`}>{unit}</button>
                    ))}
                  </div>
                </div>
                <div className="flex gap-3 pt-6">
                  <button onClick={() => setRecordStep(1)} className="flex-1 py-4 text-xs font-medium border border-zinc-100 rounded-xl hover:bg-zinc-50 transition-colors uppercase tracking-widest text-[#888888]">이전</button>
    ...
    return (
      <div className="space-y-8">
        ...
        <div className="flex gap-3 pt-6">
          ...
          <button disabled={!recordData.memo || isSubmitting} onClick={handleRecordActivity} className="flex-[2] py-4 text-sm font-medium bg-black text-white rounded-xl active:scale-[0.98] transition-all disabled:bg-zinc-100 disabled:text-[#888888]">
            {isSubmitting ? "기록 중..." : "기록 완료"}
          </button>
        </div>
      </div>
    ...
    <main className="min-h-screen bg-white">
      <nav className="flex justify-between items-center px-6 py-8">
        ...
        <div className="flex items-center gap-6 relative">
                  ...
                  <>
                    ...
                    <motion.div initial={{ opacity: 0, y: 5, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 5, scale: 0.98 }} className="fixed top-24 left-1/2 -translate-x-1/2 sm:absolute sm:top-full sm:right-0 sm:left-auto sm:translate-x-0 sm:mt-3 w-[340px] sm:w-[260px] bg-white z-[70] shadow-[0_20px_50px_rgba(0,0,0,0.12)] rounded-3xl border border-zinc-100 overflow-hidden">
                      <div className="px-4 py-3 border-b border-zinc-50 flex justify-between items-center">
                        <p className="text-[10px] font-semibold tracking-widest uppercase text-[#1A1A1A] border-l border-black pl-2">새로운 기록 알림</p>
                      </div>
                      <div className="max-h-[300px] overflow-y-auto custom-scrollbar p-2">
                        {recentNotifications.length > 0 ? (
                          ...
                          <div className="py-10 text-center flex flex-col items-center gap-2">
                            ...
                            <p className="text-[10px] text-[#888888] font-semibold uppercase tracking-widest">새로운 알림이 없습니다</p>
                          </div>
                        )}
                      </div>
                    </motion.div>
            ...
            <div className="relative">
              <button onClick={() => setIsMailboxOpen(!isMailboxOpen)} className="text-zinc-200 hover:text-black transition-colors flex items-center justify-center p-1 relative">
                ...
                {(() => {
                  ...
                  if (unreadCount > 0) {
                    return (
                      <span className="absolute 0 -right-1 w-3.5 h-3.5 bg-[#1A1A1A] rounded-full flex items-center justify-center text-[8px] font-semibold text-white shadow-sm ring-[1.5px] ring-white">
                        {unreadCount}
                      </span>
                    );
                  }
                    ...
                    <motion.div initial={{ opacity: 0, y: 5, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 5, scale: 0.98 }} className="fixed top-24 left-1/2 -translate-x-1/2 sm:absolute sm:top-full sm:right-0 sm:left-auto sm:translate-x-0 sm:mt-3 w-[340px] sm:w-[320px] bg-[#fffcf5] z-[70] shadow-[0_20px_50px_rgba(0,0,0,0.12)] rounded-3xl border border-[#efe9dc] overflow-hidden">
                      {selectedLetter ? (
                        <div className="p-5 relative min-h-[300px] flex flex-col items-center text-center">
                          ...
                          <h4 className="text-sm font-semibold text-[#3d3730] mb-6">{selectedLetter.title}</h4>
                          <p className="text-[11px] text-[#5c544a] leading-[1.8] whitespace-pre-wrap font-medium">{selectedLetter.content}</p>
                          <p className="text-[9px] text-[#888888] font-mono mt-8 mb-2 absolute bottom-2">{new Date(selectedLetter.date).toLocaleDateString()}</p>
                        </div>
                      ) : (
                        <>
                          <div className="px-5 py-4 border-b border-[#efe9dc] flex justify-between items-center bg-[#fdfaf3]">
                            <p className="text-[11px] font-semibold tracking-widest uppercase text-[#5c544a] flex items-center gap-2"><span className="text-lg">💌</span> 우리 아이들의 편지함</p>
                          </div>
                          <div className="max-h-[350px] overflow-y-auto custom-scrollbar p-2">
                            {mailboxLetters.length > 0 ? (
                                ...
                                <button key={letter.id} onClick={() => handleReadLetter(letter)} className="w-full p-4 hover:bg-[#f6ebd8]/50 rounded-2xl transition-colors space-y-2 flex gap-4 items-center group text-left">
                                  ...
                                  <div className="space-y-1 flex-1 min-w-0">
                                    <div className="flex justify-between items-center pr-1">
                                      <span className="text-[10px] font-semibold text-[#8c7e6d]">{letter.petName}</span>
                                      <span className="text-[8px] text-[#b3a89e] font-mono">{new Date(letter.date).toLocaleDateString()}</span>
                                    </div>
                                    <p className={`text-[12px] truncate pr-2 ${!readLetters.includes(letter.id) ? 'text-[#3d3730] font-semibold' : 'text-[#8c7e6d] font-normal'}`}>
                              ...
                              <div className="py-12 text-center flex flex-col items-center gap-3">
                                ...
                                <p className="text-[10px] text-[#888888] font-semibold uppercase tracking-widest">아직 도착한 편지가 없어요</p>
                              </div>
                            )}
                          </div>
                        </>
          ...
          {profile?.role === 'admin' && (
            <Link href="/dashboard/admin" className="text-[10px] tracking-widest text-[#888888] uppercase hover:text-black transition-colors">
              Admin
            </Link>
          )}
          <div className="relative h-8 flex items-center">
            <button onClick={() => { setIsMyPageOpen(!isMyPageOpen); setActiveMyPageTab('root'); }} className={`text-[10px] tracking-[0.2em] uppercase transition-colors leading-none ${isMyPageOpen ? 'text-black font-semibold' : 'text-[#888888] hover:text-black'}`}>마이페이지</button>
            <AnimatePresence>
              {isMyPageOpen && (
                <>
                      ...
                      {activeMyPageTab === 'root' ? (
                        <div className="space-y-0.5">
                          <Link href="/dashboard/profile" className="w-full text-left p-2.5 rounded-xl hover:bg-zinc-50 transition-all flex justify-between items-center group">
                            <div className="flex flex-col">
                              <span className="text-[10px] font-semibold">{profile?.nickname || profile?.name || '사용자'}</span>
                              <span className="text-[8px] text-[#888888]">나의 정보 수정</span>
                            </div>
                        ...
                        <div className="p-1.5 space-y-4 animate-in fade-in slide-in-from-right-1 duration-200">
                          ...
                          <div className="space-y-3 px-1">
                            <div className="space-y-1">
                              <label className="text-[8px] text-[#888888] uppercase tracking-widest pl-1 font-mono">닉네임</label>
                            ...
                            <div className="space-y-1">
                              <label className="text-[8px] text-[#888888] uppercase tracking-widest pl-1 font-mono">전화번호</label>
                            ...
                            <button disabled={isSubmitting} onClick={handleUpdateProfile} className="w-full py-3 bg-black text-white rounded-xl text-[10px] font-semibold mt-2 disabled:bg-zinc-200">저장하기</button>
                          </div>
                        </div>
                      ) : (
                          ...
                          <div className="space-y-2.5">
                            <div className="p-3 bg-zinc-50 rounded-2xl space-y-3">
                              <div className="space-y-1.5 flex flex-col">
                                <span className="text-[7px] text-[#888888] uppercase tracking-widest font-mono pl-1">가족 그룹 이름</span>
                              ...
                              <div className="pt-3 border-t border-zinc-100 space-y-1">
                                ...
                                <div className="flex items-center justify-between px-1">
                                  <p className="text-[11px] font-mono font-black text-[#1A1A1A] tracking-wider font-semibold"> {profile?.families?.invite_code} </p>
                                  <button onClick={() => { navigator.clipboard.writeText(profile?.families?.invite_code || ''); alert("초대 코드가 복사되었습니다."); }} className="text-[8px] text-[#888888] hover:text-black uppercase tracking-tighter">복사</button>
                                </div>
                              </div>
                            </div>
                              ...
                              {familyMembers.length > 0 ? (familyMembers.map((member: any) => (
                                <div key={member.id} className="flex items-center gap-2 p-2 rounded-lg border border-zinc-50 bg-white">
                                  ...
                                  {profile?.families?.created_by === profile?.id && member.id !== profile?.id && (
                                    <div className="flex gap-2">
                                      <button onClick={() => handleTransferLeadership(member.id, member.nickname || member.name)} className="text-[8px] text-[#888888] hover:text-blue-500 transition-colors font-medium border-r border-zinc-100 pr-2">위임</button>
      ...
      <div className="max-w-[800px] mx-auto px-6 pt-5 pb-28 space-y-12">
        <section className="space-y-4">
                  ...
                  <>
                    ...
                    <motion.div initial={{ opacity: 0, y: 10, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 10, scale: 0.95 }} transition={{ duration: 0.2, ease: "easeOut" }} className="absolute top-full left-0 mt-3 w-full max-w-[280px] bg-white z-[70] shadow-[0_20px_50px_rgba(0,0,0,0.12)] rounded-3xl border border-zinc-100 overflow-hidden p-2">
                      <div className="p-1 space-y-1">
                        <p className="px-3 py-2 text-[8px] text-[#888888] uppercase tracking-widest font-semibold">나의 가족 그룹</p>
                        {myFamilies.length > 0 ? (
                          <div className="space-y-1 max-h-[240px] overflow-y-auto pr-1 custom-scrollbar">
                            {myFamilies.map((fam: any) => (
                              <button key={fam.id} onClick={() => handleSwitchFamily(fam.id, fam.name)} className={`w-full flex items-center justify-between p-4 rounded-2xl transition-all text-left ${fam.id === profile?.family_id ? 'bg-zinc-900 text-white' : 'hover:bg-zinc-50 text-[#1A1A1A]'}`}>
                                <div>
                                  ...
                                  <p className={`text-[9px] font-mono mt-0.5 ${fam.id === profile?.family_id ? 'text-[#888888]' : 'text-zinc-300'}`}>CODE: {fam.invite_code}</p>
                                </div>
          ...
          <div className="space-y-3">
            ...
            <p className="text-[11pt] text-[#888888] font-light">사랑스러운 나의 {pets.length}마리의 아이들을 돌봐주세요 🫶🏻</p>
          </div>
        </section>
              ...
              <div className="relative flex items-center justify-center h-[280px] sm:h-[320px]">
                ...
                <div className="relative w-full max-w-[320px] sm:max-w-[420px] mx-auto">
                  ...
                  {petsOrder.slice(0, 3).map((id, index) => {
                    ...
                    const isAddCard = id === 'add-pet-card';
                        ...
                        {isAddCard ? (
                          <Link href="/dashboard/add-pet" className="w-full h-full flex flex-col items-center justify-center gap-6 group">
                            ...
                            <div className="text-center space-y-1">
                              <p className="text-xs font-semibold text-[#1A1A1A]">새로운 아이 등록</p>
                              <p className="text-[9px] text-zinc-300 uppercase tracking-widest font-mono">Add New Pet</p>
                            </div>
                          </Link>
                        ) : (
                          <>
          ...
          <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-[110] flex items-center justify-center p-6 animate-in fade-in duration-300">
            ...
            >
              <div className="flex justify-between items-center mb-8">
                <div className="flex items-center gap-4">
                  {pickerStep === 'day' && (
                    <button onClick={() => setPickerStep('month')} className="p-2 -ml-2 text-[#888888] hover:text-[#1A1A1A] transition-colors">
                      <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg>
                    </button>
                  )}
                  {pickerStep === 'month' ? (
                    <div className="flex items-center gap-3">
                      <button onClick={() => setPickerYear(prev => prev - 1)} className="p-2 hover:bg-zinc-50 rounded-lg text-zinc-300 hover:text-black transition-colors">
                      ...
                      <span className="text-xl font-semibold tracking-tighter">{pickerYear}년</span>
                      <button onClick={() => setPickerYear(prev => prev + 1)} className="p-2 hover:bg-zinc-50 rounded-lg text-zinc-300 hover:text-black transition-colors">
              ...
              {pickerStep === 'month' ? (
                <div className="grid grid-cols-3 gap-3">
                  {Array.from({ length: 12 }, (_, i) => i + 1).map(month => {
                    const isCurrent = historySelectedDate.getFullYear() === pickerYear && historySelectedDate.getMonth() + 1 === month;
                    return (
                      <button
                        key={month}
                        onClick={() => {
                      ...
                      for (let d = 1; d <= daysInMonth; d++) {
                        ...
                        days.push(
                          <button
                            key={d}
                            onClick={() => {
                              setHistorySelectedDate(dateObj);
                              setIsDatePickerOpen(false);
                              setPickerStep('month');
                            }}
              ...
              <div className="mt-8 pt-6 border-t border-zinc-50 flex justify-center">
                <button
                  ...
                  className="text-[10px] text-zinc-300 uppercase tracking-widest hover:text-black transition-colors font-semibold"
                >
                  오늘로 이동하기
                </button>
              </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-zinc-100 z-[100] pb-safe shadow-[0_-5px_20px_rgba(0,0,0,0.03)]">
        <nav className="max-w-[800px] mx-auto px-6 py-2 flex justify-between items-center">
          <button onClick={() => { setActiveBottomTab('home'); setTimeout(() => window.scrollTo({ top: 0, behavior: 'smooth' }), 50); }} className={`flex flex-col items-center gap-1.5 transition-colors flex-1 py-1 sm:py-2 ${activeBottomTab === 'home' ? 'text-black' : 'text-[#888888] hover:text-zinc-600'}`}>
          ...
          <button onClick={() => { setActiveBottomTab('pocket'); setTimeout(() => window.scrollTo({ top: 0, behavior: 'smooth' }), 50); }} className={`flex flex-col items-center gap-1.5 transition-colors flex-1 py-1 sm:py-2 ${activeBottomTab === 'pocket' ? 'text-black' : 'text-[#888888] hover:text-zinc-600'}`}>
          ...
          <button onClick={() => { setActiveBottomTab('calendar'); setTimeout(() => window.scrollTo({ top: 0, behavior: 'smooth' }), 50); }} className={`flex flex-col items-center gap-1.5 transition-colors flex-1 py-1 sm:py-2 ${activeBottomTab === 'calendar' ? 'text-black' : 'text-[#888888] hover:text-zinc-600'}`}>
